...
import os, random, string, datetime, logging, sys, re, requests, time, json
from langchain.document_loaders import PDFPlumberLoader, BSHTMLLoader, WebBaseLoader
...
def chat_with_web(hash_name):
    if request.method == 'POST':
        ...
        if os.path.exists(os.path.join(os.getcwd(), 'uploads', f"{hash_name}.json")):
            # file_path = os.path.join(os.getcwd(), 'uploads', f"{hash_name}.html")
            file_path = os.path.join(os.getcwd(), 'uploads', f"{hash_name}.json")
            with open(file_path, 'r') as txt:
                data = json.load(txt)
            url = data.get(f'{hash_name}')
...
def htmlurl(url):
    html_name = generate_random_hex_name()
    path = {f"{html_name}":url}
    file_path = os.path.join(os.getcwd(),'uploads',f'{html_name}.json')
    with open(file_path, 'w') as txt:
        json.dump(path,txt, indent=1)
    return html_name
